-- 0073_fn_submit_draft_v5_fix_delegation_columns.sql
--
-- 0054는 의도된 migration이었으나 주석만 남기고 실제 CREATE FUNCTION을
-- MCP로만 적용해 프로덕션/신규 설치 DB에 반영되지 않음.
-- 신규 설치(2026-04-14 Ubuntu+Podman 배포 과정에서 발견)에서
-- `column "delegator_id" does not exist` 오류로 품의서 상신 전부 실패.
--
-- 실제 delegation_rules 스키마 (0031~0034에서 확정):
--   delegator_user_id  (not delegator_id)
--   delegate_user_id   (not delegate_id)
--   effective_to       (not effective_until)
--   is_active 컬럼 없음 → 조건 제거
--
-- 0045의 fn_submit_draft v4를 베이스로 delegation 쿼리만 수정한 v5.
-- 나머지 로직 동일 (auto approval line, groupOrder 연속성 검증, audit 로그 등).
...
BEGIN
  ...
  -- 7. steps 생성 + delegation 매칭 (v5: 컬럼명 수정)
  v_index := 0;
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_approval_line) LOOP
    ...
    -- delegation 매칭 (v5: delegator_user_id / effective_to / is_active 제거)
    SELECT * INTO v_deleg
    ...
    IF v_deleg.id IS NOT NULL AND v_step_type = 'approval' THEN
      -- delegation skip (v5: delegate_user_id)
      INSERT INTO public.approval_steps
        (document_id, step_index, group_order, step_type, approver_user_id, status, comment)
      VALUES
        (v_doc.id, v_index, v_group_order, v_step_type, v_step_user, 'skipped', '[전결] ' || COALESCE(v_deleg.delegate_user_id::text, ''));
