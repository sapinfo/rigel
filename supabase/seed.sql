...
-- ============================================================
-- 18. 결재 문서 샘플 (seed_create_document 헬퍼 사용)
-- 함수는 0070_seed_helper_function.sql migration에서 생성
-- 나중에 새 문서 추가: SELECT seed_create_document(...) 한 줄이면 끝
-- ============================================================

-- DOC1: 완료 — 일반 기안서 (2단계 전체 승인, 5일 전)
SELECT seed_create_document(
  p_tenant_id := 'b0000000-0000-0000-0000-000000000001',
  p_form_code := 'general',
  p_drafter_id := 'a0000000-0000-0000-0000-000000000003',
  p_title := '사무용품 구매 요청',
  p_content := '{"title":"사무용품 구매 요청","summary":"모니터 거치대 5개 구매","body":"개발팀에서 사용할 모니터 거치대 5개를 구매 요청합니다."}'::jsonb,
  p_approvers := ARRAY['a0000000-0000-0000-0000-000000000002','a0000000-0000-0000-0000-000000000001']::uuid[],
  p_target_status := 'completed',
  p_days_ago := 5
);

-- DOC2: 진행 중 — 휴가신청서 (1단계 승인, 2단계 대기, 2일 전)
SELECT seed_create_document(
  p_tenant_id := 'b0000000-0000-0000-0000-000000000001',
  p_form_code := 'leave-request',
  p_drafter_id := 'a0000000-0000-0000-0000-000000000003',
  p_title := '연차 사용',
  p_content := '{"leave_type":"annual","period":"2026-04-21 ~ 2026-04-22","reason":"개인 사유로 연차 사용","contact":"010-3456-7890"}'::jsonb,
  p_approvers := ARRAY['a0000000-0000-0000-0000-000000000002','a0000000-0000-0000-0000-000000000001']::uuid[],
  p_target_status := 'in_progress',
  p_days_ago := 2
);

-- DOC3: 반려 — 지출결의서 (1단계에서 반려, 3일 전)
SELECT seed_create_document(
  p_tenant_id := 'b0000000-0000-0000-0000-000000000001',
  p_form_code := 'expense-report',
  p_drafter_id := 'a0000000-0000-0000-0000-000000000002',
  p_title := '접대비 청구',
  p_content := '{"title":"접대비 청구","expense_date":"2026-04-08","amount":350000,"category":"entertainment","description":"거래처 저녁 식사"}'::jsonb,
  p_approvers := ARRAY['a0000000-0000-0000-0000-000000000001']::uuid[],
  p_target_status := 'rejected',
  p_reject_at_step := 0,
  p_reject_comment := '영수증이 첨부되지 않았습니다. 재기안 바랍니다.',
  p_days_ago := 3
);
...
SELECT seed_create_document(
  p_tenant_id := 'b0000000-0000-0000-0000-000000000001',
  p_form_code := 'proposal',
  p_drafter_id := 'a0000000-0000-0000-0000-000000000002',
  p_title := 'AWS 서버 증설 품의',
  p_content := '{"title":"AWS 서버 증설 품의","purpose":"트래픽 증가 대비","budget":5000000,"justification":"월 사용자 300% 증가 예상"}'::jsonb,
  p_target_status := 'draft',
  p_days_ago := 0
);

-- DOC5: 긴급 진행 중 — 업무협조전 (합의 스텝 포함, 1일 전)
SELECT seed_create_document(
  p_tenant_id := 'b0000000-0000-0000-0000-000000000001',
  p_form_code := 'cooperation',
  p_drafter_id := 'a0000000-0000-0000-0000-000000000002',
  p_title := '보안 패치 긴급 적용 요청',
  p_content := '{"title":"보안 패치 긴급 적용 요청","target_department":"경영지원팀","deadline":"2026-04-15","content":"CVE-2026-1234 취약점 패치를 긴급 적용해야 합니다."}'::jsonb,
  p_approvers := ARRAY['a0000000-0000-0000-0000-000000000001','a0000000-0000-0000-0000-000000000003']::uuid[],
  p_target_status := 'in_progress',
  p_urgency := '긴급',
  p_step_types := ARRAY['approval','agreement'],
  p_days_ago := 1
);
...
SELECT seed_create_document(
  ...
  p_drafter_id := 'a0000000-0000-0000-0000-000000000003',
  p_title := '출장 보고서 (취소)',
  p_content := '{"title":"출장 보고서 (취소)","summary":"서울 출장 보고","body":"출장이 취소되어 회수합니다."}'::jsonb,
  p_approvers := ARRAY['a0000000-0000-0000-0000-000000000002']::uuid[],
